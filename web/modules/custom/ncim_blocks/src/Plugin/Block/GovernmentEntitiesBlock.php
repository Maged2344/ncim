...
/**
 * Provides a "Government Entities Page Content" block.
 *
 * @Block(
 *   id = "government_entities_content",
 *   admin_label = @Translation("Government Entities Page Content"),
 *   category = @Translation("NCIM Blocks")
 * )
 */
class GovernmentEntitiesBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs a new GovernmentEntitiesBlock instance.
   *
  ...
  public function build() {
    ...
    $page_content = $this->loadGovernmentEntitiesPage();
    if ($page_content) {
      $content['page'] = $this->buildPageContent($page_content);
    }

    // Load beneficiary types.
    $beneficiaries = $this->loadGovernmentBeneficiaryTypes();
    if ($beneficiaries) {
    ...
    if (empty($content)) {
        return [
          '#markup' => '<p>' . $this->t('Government entities page content not found.') . '</p>',
        ];
    }

    return [
      '#theme' => 'government_entities_content',
      '#content' => $content,
      '#cache' => [
        'max-age' => 0,
      ],
  ...
  /**
   * Load the government entities page content.
   */
  protected function loadGovernmentEntitiesPage() {
    $current_language = \Drupal::languageManager()->getCurrentLanguage();
    $language_id = $current_language->getId();
    $query = $this->entityTypeManager->getStorage('node')->getQuery()
      ->condition('type', 'government_entities_page')
      ->condition('status', 1)
  ...
  /**
   * Load beneficiary types for government entities.
   */
  protected function loadGovernmentBeneficiaryTypes() {
    $current_language = \Drupal::languageManager()->getCurrentLanguage();
    $language_id = $current_language->getId();
    
    // Query for beneficiary types that should appear in government entities block
    $query = $this->entityTypeManager->getStorage('node')->getQuery()
    ...
    if ($this->fieldExists('node', 'beneficiary_type', 'field_display_location')) {
      $query->condition('field_display_location', ['government_entities', 'both'], 'IN');
    }
    ...
    if ($this->fieldExists('node', 'beneficiary_type', 'field_display_location')) {
      $fallback_query->condition('field_display_location', ['government_entities', 'both'], 'IN');
    }
